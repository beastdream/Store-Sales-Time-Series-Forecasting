...
POWER_BI_PATH = PROJECT_ROOT / "powerbi" / "store_sales_analytics.pbix"
...
def _check_power_bi_artifact(outcome: ValidationOutcome) -> None:
    exists = POWER_BI_PATH.is_file() and POWER_BI_PATH.stat().st_size > 0
    outcome.results.append(
        CheckResult(
            "Power BI dashboard artifact",
            "PASS" if exists else "FAIL",
            (
                f"{POWER_BI_PATH.relative_to(PROJECT_ROOT).as_posix()} exists and "
                f"is non-empty ({POWER_BI_PATH.stat().st_size:,} bytes)."
                if exists
                else "The expected local PBIX artifact is missing or empty."
            ),
        )
    )
    outcome.powerbi_readiness.extend(
        [
            ...
            "Power BI Service publication, gateway, credentials, scheduled refresh, "
            "and production reconciliation are outside this file-based validator.",
        ]
    )
...
def _check_repository_hygiene(outcome: ValidationOutcome, runner: CommandRunner) -> None:
    ...
    da_parquet_roots = {DATA_INTERIM, DATA_PROCESSED, DATA_FEATURES}
    parquet_tracked = [
        path
        for path in tracked
        if path.suffix.lower() == ".parquet"
        and any(root in path.parents for root in da_parquet_roots)
    ]
    ...
    outcome.results.append(
        CheckResult(
            ...
            "Required ignore rules exist; .gitkeep is retained; no generated DA Parquet is tracked."
            if hygiene_ok
            else f"Missing rules={sorted(missing_rules)}; tracked DA Parquet={len(parquet_tracked)}.",
        )
    )
    raw_tracked = sorted(
        path.relative_to(PROJECT_ROOT).as_posix()
    ...
    outcome.git_hygiene.extend(
        [
            ...
            f"Tracked generated DA Parquet files: {len(parquet_tracked)}.",
...
def run_validation(runner: CommandRunner = _run_command) -> ValidationOutcome:
    ...
    try:
        ...
        _check_power_bi_artifact(outcome)
    except Exception as exc:
        outcome.results.append(
            CheckResult(
...
def render_report(outcome: ValidationOutcome) -> str:
    ...
    lines = [
        ...
        "",
        "> Scope: Data Engineering, descriptive analytics artifacts, repository "
        "hygiene, and local Power BI artifact existence. Forecast-model validation "
        "is owned by `src.validate_ds_project`.",
        "",
        "## Execution timestamp",
        "",
